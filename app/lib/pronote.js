 ...
 * @param {string|object} pronoteSubject - Subject name from Pronote (can be string or object with 'name' property)
...
export function mapPronoteSubject(pronoteSubject, localSubjects) {
  ...
  let subjectName;
  if (typeof pronoteSubject === 'string') {
    subjectName = pronoteSubject;
  } else if (pronoteSubject && typeof pronoteSubject === 'object') {
    // Python returns objects like {name: "Mathématiques", ...}
    subjectName = pronoteSubject.name || pronoteSubject.subject || '';
  } else {
    return null;
  }
  ...
  const cleanPronote = subjectName.toLowerCase().trim();
